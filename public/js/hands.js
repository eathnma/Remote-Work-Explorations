...
Spring.prototype.update = function () {
  ...
  if (!this.a.fixed) this.a.x += delta.x;
  if (!this.b.fixed) this.b.x -= delta.x;
};
...
export class Hands {
  ...
  updateObject(mScale, x, y, type, slap) {
    ...
    if (type === "you") {
      ...
      //attach arm to wrist
      yourArm.firstSegment.point = yourHand.firstSegment.point.add(
        new Point(-60, -50)
      );
      ...
      //attach arm to wrist
      theirArm.firstSegment.point = theirHand.firstSegment.point.add(
        new Point(60, 50)
      );
...
function createPath(springArray, strength) {
  ...
  for (var i = 0; i <= values.amount; i++) {
    ...
    if (i == 0 || i == values.amount) point.x += size.width;
    point.py = point.y;
    point.px = point.x;
