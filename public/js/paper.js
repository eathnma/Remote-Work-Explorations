...
yourhand.scale(0.6);
yourhand.rotate(-90);
yourhand.lastSegment.selected = true;
...
function createPath(strength) {
	...
	for (var i = 0; i <= values.amount; i++) {
		var segment = pork.add(new Point(i / values.amount, 0.5) * size);
...
var rightMiddle = yourhand.bounds.bottomRight / 2;

function onFrame(event) {
    var rotation = path.segments[1].point.angle;
    yourhand.rotate(1.1*(rotation - lastRotation), rightMiddle);
    lastRotation = rotation;
    
//    path.firstSegment.point = lastMousePosition;
    
    path.firstSegment.point = yourhand.firstSegment.point;
    path2.firstSegment.point = yourhand.lastSegment.point;
    
    yourhand.position = lastMousePosition + new Point(-150, 0);
	updateWave(path);
    updateWave(path2);
}

function updateWave(pork) {
	var force = 1 - values.friction * values.timeStep * values.timeStep;
	for (var i = 0, l = pork.segments.length; i < l; i++) {
		var point = pork.segments[i].point;
